...
export function DayView({ currentDate, events, onEventSelect, onEventCreate }: DayViewProps) {
  ...
  return (
    <div data-slot="day-view" className="contents">
        ...
        <div className="border-border/70 bg-muted/50 border-t">
          <div className="grid grid-cols-[3.5rem_1fr] sm:grid-cols-[4.5rem_1fr]">
            <div className="relative">
              <span className="text-muted-foreground/70 absolute bottom-0 left-0 h-5 sm:h-6 w-12 sm:w-16 max-w-full pe-2 sm:pe-3 text-right text-[10px] sm:text-xs">
                All day
              </span>
            </div>
            <div className="border-border/70 relative border-r p-1 sm:p-2 last:border-r-0">
              {allDayEvents.map((event) => {
                ...
                return (
                  <EventItem
                    ...
                    isLastDay={isLastDay}
                  >
                    <div className="text-xs sm:text-sm truncate">{event.title}</div>
                  </EventItem>
                )
              })}
            </div>
      ...
      <div className="border-border/70 grid flex-1 grid-cols-[3.5rem_1fr] sm:grid-cols-[4.5rem_1fr] border-t overflow-hidden">
        <div>
          {hours.map((hour, index) => (
            ...
            >
              {index > 0 && (
                <span className="text-muted-foreground/70 absolute -top-2 right-0 w-12 sm:w-14 max-w-full pe-2 sm:pe-3 text-right text-[10px] sm:text-xs">
                  {format(hour, "h a")}
                </span>
              )}
            </div>
        ...
        <div className="relative">
          ...
          {hours.map((hour, hourIndex) => (
            <DroppableCell
              key={hour.toString()}
              id={`day-cell-${currentDate.toISOString()}-${hour.toISOString()}`}
              date={new Date(currentDate.setHours(hour.getHours()))}
              onClick={() => {
